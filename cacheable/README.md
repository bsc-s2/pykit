...
-   `deref`: deepcopy or reference of the cache data(`True`: deepcopy of cache data, `False`: reference of cache data)

-   `key_extractor`: `LRU.__setitem__` `key` generate function, if `None` use the default method `cacheable.ProcessWiseCache.arg_str(args, argkv)`
    ...
    def cache_data(param):
        return need_cache_data.get(param, 'default_data')
