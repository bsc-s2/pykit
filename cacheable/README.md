...
- [Methods](#methods)
    - [LRU[key]](#lrukey)
    - [cacheable.get_cache_wrapper](#cacheablegetcachewrapper)
- [Author](#author)
...
@cacheable.get_cache_wrapper('cache_name', capacity=100, timeout=60, is_deepcopy=False)
def get_data(param):
...
class MethodCache(object):
    ...
    @cacheable.get_cache_wrapper('method_cache_name', capacity=100, timeout=60,
                                 is_deepcopy=False)
...
-   `is_deepcopy`: `get_cache_wrapper` return a decorator that use `is_deepcopy`
...
##  cacheable.get_cache_wrapper

**syntax**:
`cacheable.get_cache_wrapper(name, capacity=1024 * 4, timeout=60, is_deepcopy=True)`
...
@cacheable.get_cache_wrapper('name_aa', capacity=100, timeout=60, is_deepcopy=False)
def cache_aa(param):
    return need_cache_data_aa.get(param, '')

@cacheable.get_cache_wrapper('name_bb', capacity=100, timeout=60, is_deepcopy=False)
def cache_bb(param):
...
@cacheable.get_cache_wrapper('cache', capacity=100, timeout=60, is_deepcopy=False)
def get_data(key):
