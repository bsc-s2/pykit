...
class TestCacheable(unittest.TestCase):
    ...
    @cacheable.get_cache_wrapper('method_cache_data', capacity=10, timeout=4, is_deepcopy=False)
...
@cacheable.get_cache_wrapper('deepcopy_of_cache_data', capacity=100, timeout=60, is_deepcopy=True)
def get_deepcopy_of_cache_data(key):

    return need_cache_data.get(key, {})


@cacheable.get_cache_wrapper('cache_data', capacity=100, timeout=4, is_deepcopy=False)
